...
export const useWebRTCStream = () => {
    ...
    }, [getMediaStream, requestStreamPermission]);
    ...
    const switchSource = useCallback(async (newSource: StreamSource): Promise<MediaStream | null> => {
        ...
        try {
            ...
            toast.error('Failed to switch source', {
                description: errorMessage,
            });

            return null;
        }
    }, [state.isStreaming, state.source, getMediaStream]);
    ...
    const togglePip = useCallback(async (): Promise<MediaStream | null> => {
        ...
        if (state.isPipEnabled) {
            ...
            try {
                ...
                toast.success('Camera overlay enabled', {
                ...
                toast.error('Failed to enable camera overlay', {
                    description: 'Could not access camera for picture-in-picture.',
                });
                return null;
            }
        }
    }, [state.isStreaming, state.source, state.isPipEnabled]);
    ...
    const flipCamera = useCallback(async (): Promise<MediaStream | null> => {
        if (!state.isStreaming || state.source !== 'camera') {
            toast.error('Cannot flip camera', {
    ...
    }, [state.isStreaming, state.source, state.cameraFacing, getMediaStream]);
